...
var Worker = function(src, opts) {
    ...
    //
    // Expose the PID, in case its needed, such as killing.
    //
    self.pid = pid;
    ...
    var wsSrv = new WebSocketServer({version: "13",
                                     debug: false});
    wsSrv.addListener('connection', function(s) {
      wwutil.debug(1, 'Webworker master: Have connection event...');
      assert.equal(stream, undefined);
      assert.equal(msgStream, undefined);

      stream = s._req.socket;
      wwutil.debug(1, 'Webworker master: Created stream...');
    ...
    var start = function() {
      wsSrv.addListener('listening', function() {
        wwutil.debug(1, 'Webworker master: Have listening event...');
        ...
        wwutil.debug(1, 'Webworker master: spawning, exec path - ' + execPath + ', args - ' + sys.inspect(args) + ', cwd - ' + process.cwd());
        ...
        cp.stdout.on('data', function(data) {
          wwutil.debug(1, 'Webworker child (' + cp.pid + ') stdout - ' + data);
        });

        cp.stderr.on('data', function(data) {
          wwutil.debug(1, 'Webworker child (' + cp.pid + ') stderr - ' + data);
        });
        ...
        wwutil.debug(1,
                     'Webworker master: Spawned process ' + pid + ' for worker \'' + src + '\': ' +
                     execPath + ' ' + args.join(' ')
                    );

        cp.addListener('exit', function(code, signal) {
          wwutil.debug(
            'Webworker master: Process ' + pid + ' for worker \'' + src + 
          ...
          if (stream) {
            ...
            wwutil.debug(
              'Webworker master: Process ' + pid + ' exited without completing handshaking'
            );
          }
    ...
    var handleMessage = function(msg, fd) {
      if (!wwutil.isValidMessage(msg)) {
        wwutil.debug('Webworker master: Received invalid message: ' + sys.inspect(msg));
        return;
      }

      wwutil.debug(
        'Webworker master: Received message type=' + msg[0] + ', data=' + sys.inspect(msg[1])
      );
      ...
      case wwutil.MSGTYPE_CLIENT_READY:
      ...
      default:
        wwutil.debug(
          'Webworker master: Received unexpected message: ' + sys.inspect(msg)
        );
        break;
      }
    };
    ...
    var postMessageImpl = function(msgType, msg, fd) {
      ...
      if (clientReady && msgStream) {
        wwutil.debug('Webworker master: Master is sending message - ' + sys.inspect(msg));
        msgStream.send(m, fd);
      } else {
        wwutil.debug('Webworker master: Master is queuing message - ' + sys.inspect(msg));
        m.push(fd);
        msgQueue.push(m);
      }
    };
    ...
    self.terminate = function(timeout) {
        ...
        if (timeout > 0) {
            killTimeoutID = setTimeout(function() {
                ...
                wwutil.debug(
                    'Webworker master: Forcibily terminating worker process ' + pid + 
                        ' with SIGTERM'
                );
