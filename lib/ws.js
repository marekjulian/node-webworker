...
function Server(options){
  this.options = mixin({
    ...
    version: "auto",      // String:        Value must be either: draft75, draft76, auto, or 13 (rfc 6455)
  ...
  this.server.addListener("upgrade", function(req, socket, upgradeHead){
    wwutil.debug(1, 'Have upgrade event from server...');
    if( req.method == "GET" && ( "upgrade" in req.headers && "connection" in req.headers) && 
      ...
      wwutil.debug(1, 'Creating a new connection...');
  ...
  this.server.addListener("listening", function(req, res){
    wwutil.debug(1, "Server emitting listening event...");
    ws.emit("listening");
  });
